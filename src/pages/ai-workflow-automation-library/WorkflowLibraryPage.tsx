...
const workflows: Workflow[] = [
  ...
  {
    id: 50, industry: 'Ad & Marketing', industryColor: '#A78BFA',
    title: 'Automated SEO Foundation & Backlink Syndication',
    description: 'Client data → Airtable → n8n syndicates to directories + citation networks → Google Search Console pulls rank data → automated client report',
    nodes: ['Intake Form', 'Airtable', 'n8n', 'BrightLocal', 'GSC', 'Report'],
    nodeEmojis: ['📋', '🗃️', '⚡', '📍', '📈', '📄'],
    nodeBgs: ['#FF6B35', '#FFB347', '#F65B2B', '#00A86B', '#4285F4', '#A78BFA'],
    nodeShorts: ['INK', 'AT', 'n8n', 'BL', 'GSC', 'RPT'],
    tools: ['Airtable', 'n8n', 'BrightLocal', 'Google Search Console', 'GPT-4o', 'Slack'],
    timeSaved: '30 clients / 2 staff',
    steps: [
      'Client onboarding form collected, normalized into Airtable database',
      'n8n reads Airtable and pushes client data to BrightLocal for directory submission across 50+ citation sources',
      'Content syndication workflow distributes NAP-consistent content across web properties',
      'GSC API pulls indexing data, backlink counts, and rank position changes weekly',
      'GPT-4o drafts personalized rank improvement summaries per client',
      'Auto-formatted report emailed to client with GSC data embedded — zero human time',
    ],
    buildTime: '3–4 week sprint',
  },
  {
    id: 51, industry: 'Ad & Marketing', industryColor: '#A78BFA',
    title: 'Smart Bidding Architecture & Algorithmic Media Buying',
    description: 'Deep-funnel conversion events → Meta + Google algorithm training → real-time CPA analysis across ad sets → GPT-4o anomaly alerts → Slack',
    nodes: ['Conversions', 'Meta Ads', 'Google Ads', 'GPT-4o', 'Alert', 'Dashboard'],
    nodeEmojis: ['💰', '📱', '🔍', '🤖', '🔔', '📊'],
    nodeBgs: ['#22C55E', '#1877F2', '#4285F4', '#10A37F', '#EF4444', '#6366F1'],
    nodeShorts: ['CVR', 'FB', 'G', 'AI', '🔔', 'DASH'],
    tools: ['Meta Ads API', 'Google Ads API', 'n8n', 'GPT-4o', 'Looker Studio', 'Slack'],
    timeSaved: 'ROAS +40%',
    steps: [
      'Deep-funnel conversion events (purchases, qualified leads, calls) wired to Meta Pixel and Google Tag Manager',
      'n8n pulls real-time ad set performance data every 4 hours from Meta Ads API and Google Ads API',
      'GPT-4o compares Cost Per Result across all active ad sets, flags any set spending >20% above target CPA',
      'Meta AI Business Assistant integration reads underlying account data and surfaces CRO opportunities on creatives',
      'Anomaly alerts sent to Slack immediately when any ad set breaches budget or CPA thresholds',
      'Looker Studio dashboard auto-refreshes with ROAS, CPA, and CVR data — client-ready at all times',
    ],
    buildTime: '4–6 week sprint',
  },
  {
    id: 52, industry: 'Ad & Marketing', industryColor: '#A78BFA',
    title: 'Deep SERP-First Hybrid Content Generation',
    description: 'Target keyword → Perplexity maps top 10 SERP → information gap analysis → brand-voice-guided GPT-4o draft → human 10% review → publish',
    nodes: ['Keyword', 'SERP Map', 'Gap Analysis', 'GPT-4o Draft', 'Review', 'Publish'],
    nodeEmojis: ['🔑', '🔍', '🗺️', '🤖', '👤', '🚀'],
    nodeBgs: ['#8B5CF6', '#0099FF', '#F59E0B', '#10A37F', '#EC4899', '#22C55E'],
    nodeShorts: ['KW', 'SERP', 'GAP', 'AI', 'HUM', 'PUB'],
    tools: ['Perplexity API', 'GPT-4o', 'n8n', 'Notion', 'Google Search Console', 'Surfer SEO'],
    timeSaved: '90% drafting time',
    steps: [
      'Target keyword submitted via Notion content calendar or Slack command',
      'Perplexity API autonomously analyzes top 10 SERP results, maps competitor H2 structures and word counts',
      'Information gap analysis identifies topics the top 10 miss — unique angles the client can own',
      'GPT-4o fed brand voice guide + human-written seed paragraphs + gap map — outputs structured draft with all H2s',
      'Human strategist reviews draft (10% of time): injects anecdotes, verifies data points, adds internal links',
      'Final article pushed to CMS, internal links mapped, GSC URL submitted for indexing',
    ],
    buildTime: '2–3 week sprint',
  },
  {
    id: 53, industry: 'Ad & Marketing', industryColor: '#A78BFA',
    title: 'ComfyUI & Runway Commercial Video Pipelines',
    description: 'Creative brief → ComfyUI product photography → Runway 60-sec commercial → ElevenLabs multilingual dub + lip-sync → client delivery',
    nodes: ['Brief', 'ComfyUI', 'Runway', 'ElevenLabs', 'Lip Sync', 'Deliver'],
    nodeEmojis: ['📝', '🎨', '🎬', '🎙️', '👄', '📦'],
    nodeBgs: ['#FF6B35', '#7C3AED', '#000000', '#FF6B00', '#EC4899', '#059669'],
    nodeShorts: ['BRF', 'CFY', 'RWY', 'EL', 'LIP', 'OUT'],
    tools: ['ComfyUI', 'Runway Gen-3', 'Google Veo', 'ElevenLabs', 'n8n', 'Frame.io'],
    timeSaved: 'Days → Hours',
    steps: [
      'Creative brief submitted with product images, mood board, and target audience',
      'ComfyUI custom workflow generates native-feeling product photography with consistent lighting and brand palette — 50 variants in 20 minutes',
      'Best 3 hero shots sent to Runway Gen-3 Alpha for motion: product reveal, lifestyle b-roll, and 15-second ad cuts',
      'For 60-second commercials, Google Veo generates scene-by-scene video from storyboard prompts',
      'ElevenLabs dubs voiceover in target language; lip-sync layer applied to on-camera talent footage',
      'Final assets delivered to Frame.io for client review — organized by format (16:9, 9:16, 1:1) and language',
    ],
    buildTime: '4–6 week sprint',
  },
  {
    id: 54, industry: 'Ad & Marketing', industryColor: '#A78BFA',
    title: 'Social Intelligence & Algorithmic Subreddit Scraping',
    description: 'Monitor Reddit + LinkedIn + forums → Claude 3.5 scores relevance → high-signal posts + pre-drafted replies → human approval dashboard → publish',
    nodes: ['Reddit API', 'LinkedIn', 'Claude Score', 'Dashboard', 'Draft', 'Publish'],
    nodeEmojis: ['🤖', '💼', '🧠', '📋', '✍️', '📢'],
    nodeBgs: ['#FF4500', '#0A66C2', '#CC785C', '#6366F1', '#10A37F', '#22C55E'],
    nodeShorts: ['RED', 'LI', 'AI', 'DASH', 'DFT', 'PUB'],
    tools: ['Reddit API', 'Python', 'Claude 3.5 Sonnet', 'n8n', 'Airtable', 'Slack'],
    timeSaved: '10x signal-to-noise',
    steps: [
      'Custom scripts monitor configured subreddits, LinkedIn groups, and niche forums for client-specific keywords and problem statements — runs every 30 minutes',
      'Claude 3.5 Sonnet scores each post on 3 axes: relevance to client\'s ICP (0–10), engagement potential (0–10), purchase intent signal (0–10)',
      'Posts scoring above configurable threshold (default: 21/30 combined) are forwarded to the dashboard',
      'Claude drafts a contextual, helpful reply for each qualifying post — following the client\'s community guidelines and tone',
      'Human operator reviews dashboard feed: upvotes reply, edits for nuance, and clicks publish',
      'All published engagements logged to Airtable with post URL, reply, score, and outcome tracking',
    ],
    buildTime: '2–3 week sprint',
  },
  {
    id: 55, industry: 'Ad & Marketing', industryColor: '#A78BFA',
    title: 'Browser-Based No-API Automation for Legacy Enterprise Clients',
    description: 'Legacy enterprise client UI → Twin.so cloud browser navigates like a human → AI reads + writes data → workflow executes → zero API access needed',
    nodes: ['Trigger', 'Twin.so', 'GPT-4o Vision', 'Extract', 'Write Back', 'Log'],
    nodeEmojis: ['⚡', '🌐', '👁️', '📤', '✍️', '📋'],
    nodeBgs: ['#F65B2B', '#0F172A', '#10A37F', '#6366F1', '#8B5CF6', '#4A154B'],
    nodeShorts: ['TRG', 'TWN', 'VIS', 'EXT', 'WRT', 'LOG'],
    tools: ['Twin.so', 'n8n', 'GPT-4o', 'Slack', 'Airtable', 'Python'],
    timeSaved: 'Zero IT tickets',
    steps: [
      'Workflow trigger fires on schedule or via Slack command with the required task parameters',
      'Twin.so spins up a secure isolated cloud browser, navigates to the legacy enterprise system URL, and logs in using encrypted credentials',
      'GPT-4o Vision reads the screen — identifying fields, tables, buttons, and data structures without any API documentation',
      'AI agent executes the required actions: clicking through menus, reading table data, extracting proprietary client records',
      'Extracted data normalized and written back to Airtable or pushed to the client\'s preferred modern system',
      'Full audit log posted to Slack with screenshots of each action taken — every run is reviewable and reversible',
    ],
    buildTime: '3–4 week sprint',
  },
]
      ...
      <section className="mx-auto max-w-7xl px-6 py-12">
        ...
        {(() => {
          ...
          const dedicatedPageSlugs: Record<number, string> = {
            ...
            28: 'crm-ops-layer',
            50: 'automated-seo-foundation-backlink-syndication',
            51: 'smart-bidding-algorithmic-media-buying',
            52: 'deep-serp-first-hybrid-content-generation',
