...
import {
  ...
  Settings, Bot, Play, ShieldCheck, Repeat2, Zap
} from "lucide-react"
...
const steps = [
  { icon: Play, title: "1. Webhook Payload Ingest", desc: "n8n captures the Shopify order/product webhook event, validates the signature, and standardizes parameters." },
  { icon: Bot, title: "2. OpenAI/CrewAI Reasoning Agent", desc: "Expose payload content to LLM agents which check guidelines, formulate instructions, and select database tools." },
  { icon: Database, title: "3. Supabase Write Sync", desc: "Commit tracking parameters, variant updates, or client profile updates directly to secure databases." }
]

const features = [
  { icon: Zap, title: "Open-Source Performance", desc: "Hosting n8n workflows locally keeps execution costs at $0, preventing per-run premium Zapier pricing tiers." },
  { icon: ShieldCheck, title: "Strict Payload Decryption", desc: "Run integrations inside isolated containers, ensuring customer addresses are never saved on unencrypted systems." },
  { icon: Repeat2, title: "Automatic Webhook Retries", desc: "n8n captures API timeouts and retries variant updates dynamically, preventing database mismatches." }
]

const faqs = [
  { q: "Why use n8n over Zapier or Make?", a: "n8n offers an open-source, self-hosted option that eliminates the recurring costs of high-volume executions. It also lets developers design complex JavaScript code block nodes, making it perfect for custom AI integrations." },
  { q: "Can we sync n8n with LangGraph or CrewAI?", a: "Yes. n8n serves as a router that catches webhook inputs, feeds them to LangGraph or CrewAI endpoints, and passes the output back to Shopify." }
]
      ...
      <section className="max-w-6xl mx-auto px-6 pt-16 pb-20">
        ...
        <div className="max-w-3xl space-y-6">
          ...
          <p className="text-lg text-muted-foreground leading-relaxed max-w-2xl">
            Design and deploy n8n workflows that connect Shopify webhooks with OpenAI assistants, CrewAI execution blocks, and Supabase tables.
          </p>
      ...
      <section className="py-20 px-6 border-t border-border bg-muted/10">
        <div className="max-w-6xl mx-auto space-y-12">
          <div className="text-center space-y-2 max-w-xl mx-auto">
            <h2 className="text-3xl font-bold tracking-tight">The Sync Pipeline</h2>
