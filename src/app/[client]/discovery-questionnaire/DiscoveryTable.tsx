...
// ── Kovil header (shared across gate / welcome / submitted screens) ─────────────

function KovilHeader() {
  return (
    <header className="bg-white border-b border-[#E5E2D9]">
      <div className="max-w-5xl mx-auto px-6 py-4 flex items-center gap-3">
        <Image src="/kovil-logo-dark.webp" alt="Kovil AI" width={120} height={32}
...
function LoginGate({ config, onSuccess }: {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (!uid.trim() || !password.trim()) {
      setError('Please enter both your Access ID and password.')
      return
    }
    setLoading(true)
    setError('')
    try {
      const res = await fetch('/api/discovery/auth', {
        ...
        body: JSON.stringify({ client_slug: config.clientSlug, uid: uid.trim(), password }),
      })
      if (res.ok) {
        onSuccess()
      } else {
          ...
          <div className="bg-white rounded-2xl border border-[#E5E2D9] shadow-sm overflow-hidden">
            ...
            <div className="p-8">
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-[#0A0A0A] mb-1.5 uppercase tracking-wide">
                    Access ID
                  </label>
                  <input
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-[#E5E2D9] rounded-lg px-4 py-2.5 text-sm text-[#0A0A0A] focus:outline-none focus:border-[#FF4F00] transition-colors"
                  />
                </div>

                {error && (
                  <div className="text-xs text-red-600 bg-red-50 border border-red-100 rounded-lg px-4 py-2.5 leading-relaxed">
                    {error}
                  </div>
                )}
                ...
                <button
                  ...
                  className="w-full bg-[#FF4F00] hover:bg-[#e64600] disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold py-3 rounded-lg text-sm transition-colors flex items-center justify-center gap-2 mt-2"
                >
                  {loading ? (
                    <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Verifying…</>
                  ) : (
                    <>Access Questionnaire <span>→</span></>
                  )}
                </button>
              </form>
...
function WelcomeEmail({ config, totalQuestions, tabCount, onComplete }: {
  config: DiscoveryConfig
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-xl">
          <div className="bg-white rounded-2xl border border-[#E5E2D9] shadow-sm overflow-hidden">
            ...
            <div className="p-8">
              <div className="mb-6">
                <p className="text-xs font-bold text-[#FF4F00] uppercase tracking-widest mb-2">
                  Discovery Questionnaire
                </p>
                <h1 className="font-display text-2xl font-bold text-[#0A0A0A] leading-tight">
                  {config.projectTitle}
                </h1>
                <p className="text-sm text-gray-500 mt-1">{config.clientName}</p>
              </div>

              {/* What to expect */}
              <div className="bg-[#FAF8F4] rounded-xl border border-[#E5E2D9] p-4 mb-6 space-y-2">
                <p className="text-xs font-bold text-[#0A0A0A] uppercase tracking-wider mb-3">
                  What to expect
                </p>
                {[
                  [`📋`, `${totalQuestions} questions across ${tabCount} sections`],
                  [`💾`, `Auto-saved every 30 s — close and return any time`],
                  [`📎`, `Attach documents directly to your responses`],
              ...
              <form onSubmit={handleStart} className="space-y-4">
                <p className="text-sm font-semibold text-[#0A0A0A]">
                  Before you start, please provide your contact details so we can send you a copy of your completed questionnaire.
                </p>

                <div>
                  <label className="block text-xs font-semibold text-[#0A0A0A] mb-1.5 uppercase tracking-wide">
                    Your Name <span className="text-gray-400 normal-case font-normal">(optional)</span>
                  </label>
                  <input
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#0A0A0A] mb-1.5 uppercase tracking-wide">
                    Your Email <span className="text-[#FF4F00]">*</span>
                  </label>
                  <input
                  ...
                  <p className="text-[11px] text-gray-400 mt-1">
                    A copy of your completed questionnaire will be emailed to you.
                  </p>
                </div>
                ...
                <button
                  type="submit"
                  className="w-full bg-[#FF4F00] hover:bg-[#e64600] text-white font-bold py-3 rounded-lg text-sm transition-colors flex items-center justify-center gap-2 mt-2"
                >
                  Start Questionnaire
      ...
      <div className="flex-1 py-10 px-6">
        <div className="max-w-2xl mx-auto space-y-4">
              ...
              <div className="text-left space-y-5">
                {[
                  ...
                  },
                  {
                    ...
                    body: 'We deliver a detailed proposal, solution architecture, and SOW — tailored directly to the inputs you provided here.',
                  },
                ].map(step => (
                  <div key={step.n} className="flex gap-4">
                    <div className={`w-9 h-9 rounded-full ${step.color} text-white flex items-center justify-center text-sm font-bold flex-shrink-0 mt-0.5`}>
                      {step.n}
                    </div>
                    <div>
                      <p className="font-semibold text-[#0A0A0A] text-[15px]">{step.title}</p>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { value: String(answeredCount), label: 'Questions answered', color: 'text-emerald-600' },
              { value: String(totalCount), label: 'Total questions', color: 'text-[#0A0A0A]' },
              { value: `${pct}%`, label: 'Completion rate', color: pct === 100 ? 'text-emerald-600' : 'text-[#FF4F00]' },
            ].map(s => (
              <div key={s.label} className="bg-white border border-[#E5E2D9] rounded-xl p-4 text-center">
          ...
          <p className="text-center text-xs text-gray-300 font-mono pt-2">Session: {sessionId}</p>
        </div>
      </div>
    </div>
  )
...
function AnswerCell({ value, placeholder, onClick, attachmentCount = 0 }: {
  value: string; placeholder: string; onClick: () => void; attachmentCount?: number
}) {
        ...
        {isEmpty ? (
          ...
          <>
            <div className="flex items-start justify-between gap-2 w-full">
              <p className="text-sm text-[#0A0A0A] leading-relaxed whitespace-pre-wrap line-clamp-5 flex-1">{value}</p>
...
function EditorPopup({
  ...
  questionId: string; field: 'response' | 'notes'; questionText: string
  initialValue: string; initialAttachments: QuestionAttachment[]; sessionId: string
  ...
  function insertBulletLine() {
    const ta = taRef.current; if (!ta) return
    const start = ta.selectionStart, before = value.slice(0, start)
    const lineStart = before.lastIndexOf('\n') + 1
    if (value.slice(lineStart, start).trim() === '') {
      const next = value.slice(0, lineStart) + '• ' + value.slice(lineStart); setValue(next)
  ...
  function insertNumberLine() {
    ...
    const lineStart = before.lastIndexOf('\n') + 1
    if (value.slice(lineStart, start).trim() === '') {
      const next = value.slice(0, lineStart) + '1. ' + value.slice(lineStart); setValue(next)
  ...
  function wrapBold() {
    ...
    if (selected) setValue(value.slice(0, start) + '**' + selected + '**' + value.slice(end))
    else insertAtCursor('**bold**')
  }
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLTextAreaElement>) {
    ...
    if (e.key === 'Enter' && !e.shiftKey) {
      ...
      const m = lastLine.match(/^(•\s|(\d+)\.\s)/)
      if (m) { e.preventDefault(); insertAtCursor('\n' + (m[2] ? `${parseInt(m[2]) + 1}. ` : '• ')) }
    }
  }

  function handleFileSelect(files: FileList | null) {
    if (!files) return
    setPendingFiles(prev => [...prev, ...Array.from(files).filter(f => f.size <= 10 * 1024 * 1024)])
  ...
  async function handleSave() {
    ...
    if (pendingFiles.length > 0 && field === 'response') {
      ...
      for (const file of pendingFiles) {
        ...
        fd.append('file', file); fd.append('session_id', sessionId); fd.append('question_id', questionId)
        try {
          ...
          if (res.ok) {
            ...
            finalAttachments.push({ name: data.name, size: data.size, type: data.type, url: data.url, uploadedAt: new Date().toISOString() })
          }
        } catch { /* silent */ }
      }
      setUploading(false)
  ...
  return (
    <div
      ...
      // Only close if BOTH mousedown AND mouseup happened on the overlay — prevents
    ...
    >
      ...
      <div
        ...
        onDrop={e => { e.preventDefault(); setIsDragOver(false); if (field === 'response') handleFileSelect(e.dataTransfer.files) }}
      >
        {isDragOver && (
        ...
        <div className="flex items-center justify-between px-5 py-3.5 bg-[#0A0A0A]">
          <div className="flex items-center gap-3 min-w-0">
            <span className="font-mono text-xs font-bold bg-[#FF4F00] text-white px-2.5 py-1 rounded flex-shrink-0">{questionId}</span>
            <span className="text-sm font-semibold text-white capitalize">
              {field === 'response' ? 'Response' : 'Notes / Add anything'}
            </span>
          </div>
          <button onClick={onClose} className="text-white/40 hover:text-white text-2xl leading-none w-7 h-7 flex items-center justify-center rounded hover:bg-white/10 transition-colors">×</button>
        </div>
        ...
        <div className="flex items-center gap-0.5 px-3 py-1.5 border-b border-[#E5E2D9] bg-white">
          {[{ label: '• Bullet', action: insertBulletLine }, { label: '1. Number', action: insertNumberLine }].map(btn => (
            <button key={btn.label} onClick={btn.action} className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium rounded hover:bg-[#EBE8E0] text-[#0A0A0A] transition-colors">{btn.label}</button>
          ))}
          <div className="w-px h-4 bg-[#E5E2D9] mx-1" />
          <button onClick={wrapBold} className="px-3 py-1.5 text-xs font-bold rounded hover:bg-[#EBE8E0] text-[#0A0A0A] transition-colors">B</button>
          <button onClick={() => insertAtCursor('\n──────────────\n')} className="px-3 py-1.5 text-xs font-medium rounded hover:bg-[#EBE8E0] text-[#0A0A0A] transition-colors">— Line</button>
        ...
        {/* Textarea */}
        <textarea
          ref={taRef} value={value} onChange={e => setValue(e.target.value)} onKeyDown={handleKeyDown}
          placeholder={field === 'response'
          ...
          <div className="border-t border-[#E5E2D9] bg-[#FAF8F4] px-5 py-3">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-semibold text-[#0A0A0A]">Attachments{hasFiles ? ` (${existingAttachments.length + pendingFiles.length})` : ''}</span>
              </div>
            ...
            {hasFiles ? (
              <div className="space-y-1">
                {existingAttachments.map((att, i) => (
                  <div key={`ex-${i}`} className="flex items-center gap-2 bg-white border border-[#E5E2D9] rounded px-3 py-1.5">
                    ...
                    <a href={att.url} target="_blank" rel="noreferrer" className="text-xs text-blue-600 hover:underline truncate flex-1 min-w-0">{att.name}</a>
                    <span className="text-[11px] text-gray-400 flex-shrink-0">{formatFileSize(att.size)}</span>
                    <button onClick={() => setExistingAttachments(p => p.filter((_, j) => j !== i))} className="text-gray-300 hover:text-red-400 transition-colors text-base leading-none">×</button>
                  </div>
                ))}
                {pendingFiles.map((file, i) => (
                  <div key={`pend-${i}`} className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded px-3 py-1.5">
                    ...
                    <span className="text-xs text-[#0A0A0A] truncate flex-1 min-w-0">{file.name}</span>
                    ...
                    <button onClick={() => setPendingFiles(p => p.filter((_, j) => j !== i))} className="text-gray-300 hover:text-red-400 transition-colors text-base leading-none">×</button>
                  </div>
                ))}
                <button type="button" onClick={() => fileInputRef.current?.click()}
              ...
              <div className="border border-dashed border-[#E5E2D9] rounded-lg p-4 text-center cursor-pointer hover:border-[#FF4F00] hover:bg-orange-50/30 transition-colors"
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-[#E5E2D9] bg-white">
          <span className="text-[11px] text-gray-400">Ctrl+Enter = save · Ctrl+B = bold · Esc = cancel</span>
          <div className="flex items-center gap-2">
            <button onClick={onClose} className="px-4 py-2 text-sm rounded-lg border border-[#E5E2D9] text-gray-600 hover:bg-[#EBE8E0] transition-colors font-medium">Cancel</button>
            <button onClick={() => { void handleSave() }} disabled={saved || uploading}
              className={`flex items-center gap-2 px-5 py-2 text-sm rounded-lg font-bold transition-all duration-200 ${
                saved ? 'bg-emerald-500 text-white scale-95' : uploading ? 'bg-amber-500 text-white cursor-wait' : 'bg-[#FF4F00] text-white hover:bg-[#e64600]'
              }`}
            >
              {saved ? (<><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>Saved!</>)
                : uploading ? (<><span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />Uploading…</>)
...
function SaveIndicator({ status, lastSavedAt }: { status: SaveStatus; lastSavedAt: Date | null }) {
  if (status === 'saving') return <div className="flex items-center gap-2 text-xs text-gray-500"><span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" />Saving…</div>
  if (status === 'saved') return (
    <div className="flex items-center gap-2 text-xs text-gray-500">
      <span className="w-2 h-2 rounded-full bg-emerald-500" />All changes saved
      {lastSavedAt && <span className="text-gray-400">· {lastSavedAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>}
    </div>
  )
  if (status === 'error') return <div className="flex items-center gap-2 text-xs"><span className="w-2 h-2 rounded-full bg-red-500" /><span className="text-red-500">Server sync failed — answers stored locally</span></div>
  return <div className="flex items-center gap-2 text-xs text-gray-400"><span className="w-2 h-2 rounded-full bg-gray-300" />Auto-saves every 30 s</div>
}

// ── Tab button ─────────────────────────────────────────────────────────────────

function TabBtn({ label, count, done, active, onClick, allDone }: {
  label: string; count: number; done?: number; active: boolean; onClick: () => void; allDone?: boolean
}) {
  return (
    <button onClick={onClick}
      className={`relative px-4 py-3 text-sm font-medium whitespace-nowrap border-b-2 transition-colors ${active ? 'border-[#FF4F00] text-[#FF4F00]' : 'border-transparent text-gray-500 hover:text-[#0A0A0A] hover:border-gray-300'}`}>
      {label}
      {done !== undefined
        ? <span className={`ml-1.5 text-[11px] px-1.5 py-0.5 rounded-full font-semibold ${allDone ? 'bg-emerald-100 text-emerald-700' : active ? 'bg-orange-100 text-[#FF4F00]' : 'bg-gray-100 text-gray-500'}`}>{done}/{count}</span>
        : <span className={`ml-1.5 text-[11px] px-1.5 py-0.5 rounded-full font-semibold ${allDone ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}>{count}</span>}
    </button>
  )
}
...
export default function DiscoveryTable({ config }: { config: DiscoveryConfig }) {
  ...
  // ── Update answers ──────────────────────────────────────────────────────────
  function updateAnswer(questionId: string, field: keyof QuestionAnswer, value: string, attachments?: QuestionAttachment[]) {
  ...
  function openPopup(questionId: string, field: 'response' | 'notes', questionText: string) {
    ...
    setPopupInitialAttachments(field === 'response' ? (answers[questionId]?.attachments ?? []) : [])
        ...
        <table className="w-full">
          ...
          <tfoot>
            <tr className="border-t-2 border-[#0A0A0A] bg-[#FAF8F4]">
              ...
              <td className="py-3.5 px-4 text-center"><span className={`text-sm font-bold ${answeredCount === totalCount ? 'text-emerald-600' : 'text-[#0A0A0A]'}`}>{answeredCount}</span></td>
              <td className="py-3.5 px-5">
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-2 bg-red-100 rounded-full overflow-hidden">
  ...
  return (
    <div className="h-screen bg-white flex flex-col overflow-hidden">
      ...
      <div className="flex-shrink-0 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-4 px-5 py-2">
            ...
            <nav className="flex items-center gap-1">
              <button onClick={() => setView('questions')}
                className={`px-3.5 py-1.5 rounded text-sm font-semibold transition-colors ${view === 'questions' ? 'bg-[#FF4F00] text-white' : 'text-[#0A0A0A]/60 hover:text-[#0A0A0A] hover:bg-[#0A0A0A]/8'}`}>
                Discovery Questionnaire
              </button>
              <button onClick={() => setView('tracker')}
                className={`px-3.5 py-1.5 rounded text-sm font-medium transition-colors ${view === 'tracker' ? 'bg-[#FF4F00] text-white' : 'text-[#0A0A0A]/60 hover:text-[#0A0A0A] hover:bg-[#0A0A0A]/8'}`}>
                Completion Tracker
              </button>
            </nav>
            <div className="flex-1" />
        ...
        <div className="bg-white border-b border-[#E5E2D9] px-6 py-3 flex items-center justify-between gap-6">
          <div className="min-w-0">
            <h1 className="font-display text-xl font-bold text-[#0A0A0A] leading-tight truncate">{config.projectTitle}</h1>
            <div className="flex items-center gap-3 mt-0.5">
              <p className="text-sm text-gray-400">{config.clientName}</p>
              {clientEmail && (
                <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">
                  {clientContactName ? `${clientContactName} · ` : ''}{clientEmail}
                </span>
              )}
            </div>
          </div>
          <div className="flex items-center gap-3 flex-shrink-0">
            <div className="flex items-center gap-2 bg-[#FAF8F4] border border-[#E5E2D9] rounded-lg px-3 py-2">
              ...
              <span className={`text-sm font-bold ${mustHaveAnswered === mustHaveTotal ? 'text-emerald-600' : 'text-[#FF4F00]'}`}>{mustHaveAnswered}/{mustHaveTotal}</span>
            </div>
            <div className="flex items-center gap-2 bg-[#FAF8F4] border border-[#E5E2D9] rounded-lg px-3 py-2">
        ...
        {view === 'questions' && (
          <div className="bg-white border-b border-[#E5E2D9] overflow-x-auto shadow-sm">
            <div className="flex items-end px-5 gap-0 min-w-max">
              ...
              {config.tabs.map(tab => {
                const done = tab.questions.filter(q => (answers[q.id]?.response ?? '').trim().length > 0).length
                return <TabBtn key={tab.id} label={tab.shortLabel} count={tab.questions.length}
                  done={done} active={activeTab === tab.id} onClick={() => setActiveTab(tab.id)} allDone={done === tab.questions.length} />
              })}
            </div>
          </div>
        )}
          ...
          <table className="w-full border-collapse" style={{ minWidth: '1100px' }}>
            ...
            <tbody>
              {visibleTabs.map((tab, tabIdx) => {
                let offset = 0
                if (activeTab === 'all') for (let i = 0; i < tabIdx; i++) offset += config.tabs[i].questions.length
                ...
                return (
                  <>
                    ...
                    {tab.questions.map((q, qi) => {
                      ...
                      return (
                        <tr key={q.id} className={`border-b border-[#E5E2D9] group transition-colors ${isDone ? 'bg-white' : 'bg-white hover:bg-orange-50/10'}`}>
                          <td className="py-5 px-3 text-center align-top w-10 border-r border-[#F0EDE8]">
                            {isDone
                              ? <svg className="w-5 h-5 text-emerald-500 mx-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                          ...
                          <td className="py-5 px-3 align-top w-[76px] border-r border-[#F0EDE8]">
                            <span className="inline-block text-[11px] font-mono font-bold text-gray-600 bg-[#EBE8E0] px-2 py-0.5 rounded whitespace-nowrap">{q.id}</span>
                          </td>
                          ...
                          <td className="py-5 px-4 align-top">
                            <AnswerCell value={answer.notes} placeholder="+ Add notes…"
            ...
            <div className="flex items-center gap-4">
              ...
              <button onClick={handleSubmit} disabled={submitting}
                ...
                {submitting
                  ...
                  : <>Submit Questionnaire<svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg></>}
              </button>
            </div>
          </div>
        </div>
      ...
      {popup && (
        <EditorPopup
          questionId={popup.questionId} field={popup.field} questionText={popup.questionText}
          initialValue={popupInitial} initialAttachments={popupInitialAttachments} sessionId={sessionId}
